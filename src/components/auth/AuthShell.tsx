...
export const authClerkAppearance = {
  ...
  elements: {
    ...
    card: "w-full max-w-none border border-zinc-200 bg-white p-6 shadow-xl shadow-violet-100/60 dark:border-zinc-800 dark:bg-zinc-900 dark:shadow-none sm:p-7",
    main: "w-full",
    header: "text-left",
  ...
  return (
    <div className="inline-flex items-center gap-2">
      <div
        className={
          compact
            ? "grid h-9 w-9 place-items-center rounded-xl bg-violet-600"
            : "grid h-10 w-10 place-items-center rounded-xl bg-violet-600"
        }
      >
      ...
      {!compact && (
        <div>
          <p className="text-sm font-semibold text-zinc-950 dark:text-white">SoloOS</p>
          <p className="text-[11px] text-zinc-500">Freelance command center</p>
        </div>
      )}
    </div>
  );
            ...
            <div className="flex h-28 items-end gap-2">
              {[42, 64, 52, 78, 69, 88].map((height, index) => (
                <div key={index} className="flex h-full flex-1 flex-col justify-end">
                  <div className="rounded-t-md bg-violet-600" style={{ height: `${height}%` }} />
                </div>
              ))}
            </div>
          </div>
...
export function AuthShell({ children, mode }: { children: ReactNode; mode: "login" | "sign-up" }) {
  return (
        ...
        <section className="mx-auto w-full max-w-[420px]">
          <div className="mb-7 flex flex-col items-center text-center lg:hidden">
            ...
            <p className="mt-1 text-sm text-zinc-500">
              {mode === "sign-up" ? "Create your freelance command center" : "Freelance command center"}
            </p>
          </div>
          {children}
