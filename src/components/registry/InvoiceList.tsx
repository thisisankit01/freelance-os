...
export function InvoiceList() {
    ...
    async function loadInvoiceDetails(invoiceId: string) {
        ...
        const { data } = await supabase
            .from('invoices')
            .select('*, invoice_items(*), clients(*), users(name, email, business_name, gstin)')
            .eq('id', invoiceId)
            .single()
    ...
    async function emailInvoice(invoice: any) {
        if (!invoice.clients?.email) {
            ...
            setTimeout(() => setEmailState(s => ({ ...s, [invoice.id]: 'idle' })), 3000)
            return
        }
        ...
        try {
            ...
            setEmailState(s => ({ ...s, [invoice.id]: data.success ? 'sent' : 'error' }))
        } catch {
            setEmailState(s => ({ ...s, [invoice.id]: 'error' }))
        } finally {
            setTimeout(() => setEmailState(s => ({ ...s, [invoice.id]: 'idle' })), 3000)
    ...
    return (
        ...
        >
            ...
            <div className="px-5 py-3 border-b border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
                <p className="text-sm font-medium text-zinc-700 dark:text-zinc-300">
                    {loading ? (
                        ...
                        <>
                            <span className="font-semibold text-zinc-900 dark:text-zinc-100">{invoices.length}</span>
                            {' invoices'}
                        </>
                    )}
                </p>
            ...
            {loading && (
                <div>
                    {[1, 2, 3].map(i => (
                        <div key={i} className="flex items-center gap-3 px-5 py-3.5 border-b border-zinc-50 dark:border-zinc-800/50 last:border-0">
            ...
            {!loading && invoices.length === 0 && (
                ...
                >
                    ...
                    <div>
                        <p className="text-sm font-medium text-zinc-700 dark:text-zinc-300">No invoices found</p>
                        <p className="text-xs text-zinc-400 mt-1">Type "create invoice" to make one</p>
                    </div>
                </motion.div>
            )}
                                    ...
                                    <div className="flex items-center justify-between px-5 py-3 bg-violet-50 dark:bg-violet-900/20 border-t border-violet-100 dark:border-violet-800/50 gap-3">
                                        <div className="text-xs text-violet-700 dark:text-violet-300 space-y-0.5 flex-1 min-w-0">
                                            ...
                                            <p className="text-violet-500 dark:text-violet-400 truncate">
                                                {inv.clients?.name} · ₹{inv.total?.toLocaleString('en-IN')}
                                            </p>
                                        </div>
                                        <div className="flex items-center gap-2 flex-shrink-0">
                                        {/* Email button */}
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={`text-xs px-3 py-1.5 rounded-lg font-medium transition-colors disabled:opacity-50 ${
                                                        es === 'sent'  ? 'bg-emerald-500 text-white' :
                                                        es === 'error' ? 'bg-red-500 text-white' :
                                                ...
                                                >
                                                    {es === 'sending' ? 'Sending…' :
                                                     es === 'sent'    ? '✓ Sent!' :
                                                     es === 'error'   ? (invoiceDetails.clients?.email ? '✕ Failed' : 'No email') :
                                                     '✉ Send Email'}
                                                </button>
                                            )
                                        })()}
