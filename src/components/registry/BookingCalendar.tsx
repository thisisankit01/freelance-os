...
export function BookingCalendar() {
    ...
    const load = useCallback(async () => {
        ...
        setAppointments(raw.filter((a) => (a.status || '').toLowerCase() !== 'cancelled'))
        setLoading(false)
    ...
    useEffect(() => {
        ...
        window.addEventListener('freelanceos:appointments', onRefresh)
        return () => window.removeEventListener('freelanceos:appointments', onRefresh)
    }, [load])
    ...
    async function createAppointment() {
        ...
        const json = await res.json()
        setSaving(false)
        if (!res.ok) { flash(json.error, 'error'); return }
    ...
    async function reschedule(id: string) {
        ...
        const res = await fetch('/api/appointments', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'reschedule', id, startTime: startTime.toISOString() }),
        })
        const json = await res.json()
        if (!res.ok) { flash(json.error, 'error'); return }
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-1">
                        <button onClick={() => setCurrentMonth(d => new Date(d.getFullYear(), d.getMonth() - 1))}
                            className="w-6 h-6 flex items-center justify-center rounded text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-800 text-sm transition-colors">←</button>
                        <button onClick={() => setCurrentMonth(new Date())}
                            className="text-xs px-2 py-0.5 rounded border border-zinc-200 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors">Today</button>
                        <button onClick={() => setCurrentMonth(d => new Date(d.getFullYear(), d.getMonth() + 1))}
                            className="w-6 h-6 flex items-center justify-center rounded text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-800 text-sm transition-colors">→</button>
                    </div>
                </div>
                <button
                    onClick={() => { setShowForm(f => !f); setSelected(null) }}
                ...
                {showForm && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                        ...
                        <div className="p-4 space-y-3 bg-zinc-50/50 dark:bg-zinc-800/30">
                            <p className="text-xs font-semibold text-zinc-600 dark:text-zinc-400 uppercase tracking-wide">New Appointment</p>
                            {/* Title */}
                            <input
                            ...
                            />
                            <button onClick={createAppointment} disabled={saving || !form.title.trim()}
            ...
            <div className="p-4">
                ...
                <div className="grid grid-cols-7 mb-1.5">
                    {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(d => (
                        <div key={d} className="text-center text-[11px] font-medium text-zinc-400 dark:text-zinc-500 py-1">{d}</div>
                    ))}
                </div>
                        ...
                        {days.map(day => {
                            ...
                            return (
                                <div key={day.toISOString()}
                                    ...
                                    <p className={`text-[11px] font-semibold mb-1 ${today ? 'text-violet-600 dark:text-violet-400' : 'text-zinc-400 dark:text-zinc-500'}`}>
                                        {format(day, 'd')}
                                    </p>
                                    <div className="space-y-0.5">
                                        {dayAppts.map(appt => (
                                            <button
                                                key={appt.id}
                                                onClick={() => { setSelected(appt); setShowForm(false) }}
                                                className={`w-full text-left text-[10px] rounded px-1 py-0.5 truncate leading-tight transition-colors ${appt.source === 'google_calendar'
                                            ...
                                            >
                                                ...
                                                {format(parseISO(appt.start_time), 'h:mm')} {appt.clients?.name || appt.title}
                                            </button>
                                        ))}
                                    </div>
                                </div>
                    ...
                    {selected && (
                        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 4 }}
                            ...
                            <div className="flex items-start justify-between gap-2">
                                <div>
                                    <p className="text-sm font-semibold text-zinc-800 dark:text-zinc-200">{selected.title}</p>
                                    <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">
                                        {format(parseISO(selected.start_time), 'EEEE, MMM d, yyyy · h:mm a')}
                                        {selected.clients?.name && ` · ${selected.clients.name}`}
                                    </p>
                                    {selected.notes && (
                                        <p className="text-xs text-zinc-400 dark:text-zinc-500 mt-1 italic">"{selected.notes}"</p>
                                    )}
                                    {selected.source === 'google_calendar' && (
                                        <p className="text-xs text-blue-600 dark:text-blue-400 mt-2 leading-relaxed">
                                            Linked from Google Calendar — read-only here. To cancel or reschedule, use Google Calendar.
                                        </p>
                                    )}
                                </div>
                                <button onClick={() => { setSelected(null); setRescheduling(false) }}
                                    className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 text-lg leading-none flex-shrink-0">×</button>
                            </div>

                            {/* Reschedule form */}
                            {selected.source === 'google_calendar' ? null : rescheduling ? (
                                <div className="space-y-2">
                                    <p className="text-xs font-medium text-zinc-600 dark:text-zinc-400">Pick new date & time</p>
                                    <div className="flex gap-2">
                                        <input type="date"
                                            defaultValue={format(parseISO(selected.start_time), 'yyyy-MM-dd')}
                                        ...
                                        <input type="time"
                                            defaultValue={format(parseISO(selected.start_time), 'HH:mm')}
                                    ...
                                    <div className="flex gap-2">
                                        <button onClick={() => reschedule(selected.id)}
                                ...
                                <div className="flex gap-2">
                                    <button onClick={() => {
                                        ...
                                        setRescheduleDate(format(parseISO(selected.start_time), 'yyyy-MM-dd'))
                                        setRescheduleTime(format(parseISO(selected.start_time), 'HH:mm'))
                                    }}
                ...
                {!loading && !selected && appointments.length > 0 && (
                    <div className="mt-4 pt-4 border-t border-zinc-100 dark:border-zinc-800 space-y-1">
                        <p className="text-xs font-medium text-zinc-400 dark:text-zinc-500 mb-2">Upcoming this month</p>
                        {appointments.slice(0, 4).map(appt => (
                            <button key={appt.id} onClick={() => { setSelected(appt); setRescheduling(false) }}
                                ...
                                <span className="text-zinc-300 dark:text-zinc-600 text-sm flex-shrink-0">›</span>
                            </button>
                        ))}
                    </div>
                )}
                ...
                {!loading && appointments.length === 0 && !showForm && (
                    <div className="text-center py-8">
                        <p className="text-sm text-zinc-400 dark:text-zinc-500">No appointments this month</p>
